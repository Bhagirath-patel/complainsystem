...
$("#updateh").click(function(){
  ...
  <div class="container-fluid"  style="background-color:#778899;">
   ...
   <li class="xyz" id="department"><a href="complaint.php"><h3>Home</h3></a>
	...
	<li class="xyz" id="logout"><a href="updatehod.php?reff=1" id="deleteh" ><h3>Logout</h3></a></li>
	</ul>
     </div>
	 <?php
	 ...
	 $r=mysql_query("select * from department where dname='$departname' ");
	 if($s=mysql_fetch_array($r)){
	 ?>
<div class="container-fluid" style="background-color:#a9a9a9">
<table class="table table-hover">
<form  method="post" action="updateh.php?depart=<?=$s["dname"]?>">
 ...
 <tr><td> Department:</td><td><select name="departname">
           ...
           <tr><td>Email:</td><td><input type="email" name="email" value="<?=$s["ofemail"]?>" required="required" ></td></tr>
      <tr><td>Phone No:</td><td><input type="text" name="phno" value="<?=$s["phno"]?>" required="required"></td><tr>
        <tr><td>Education:</td><td><textarea  name="education" rows="5" cols="100" required="required"><?=$s["education"]?></textarea></td></tr>
     </table>      
